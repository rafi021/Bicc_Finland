...
<div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-2xl font-bold mb-6">Edit Event Popup</h2>
    <form action="{{ route('admin.event-popups.update', $eventPopup->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700">Event Name <span class="text-red-500">*</span></label>
                <input type="text" name="event_name" value="{{ old('event_name', $eventPopup->event_name) }}" required 
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="message" rows="3" required 
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 focus:border-primary-500 focus:ring-primary-500"
                          placeholder="Additional details about the event...">{{ old('message', $eventPopup->message) }}</textarea>
                @error('message')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="event_datetime" 
                       value="{{ old('event_datetime', $eventPopup->event_datetime->format('Y-m-d\TH:i')) }}" required 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 focus:border-primary-500 focus:ring-primary-500">
            ...
            <div class="flex items-center">
                <div class="flex items-center h-full pt-6">
                    <input type="checkbox" name="is_active" id="is_active" value="1" 
                           {{ old('is_active', $eventPopup->is_active) ? 'checked' : '' }}
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="bg-primary-600 text-white px-4 py-2 rounded-md hover:bg-primary-700">Update Event Popup</button>
        </div>
    </form>
</div>
@endsection
