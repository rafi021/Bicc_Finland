...
<div class="bg-white rounded-lg shadow-md p-6 max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold">Edit Service: {{ $service->title }}</h2>
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="description" rows="3" required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm border p-2"
                placeholder="Briefly describe the service...">{{ old('description', $service->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="content" rows="10" required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm border p-2"
                placeholder="Provide detailed information about the service...">{{ old('content', $service->content) }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="file" name="image" accept="image/*" class="dropify" data-height="150" 
                data-default-file="{{ image($service->image) }}">
        </div>
